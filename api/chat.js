...
#welcome-inner {
  width: 400px; padding: 48px 40px; text-align: center;
}
#welcome-inner h1 { font-size: 28px; font-weight: 700; letter-spacing: -1px; margin-bottom: 8px; }
#welcome-inner p { font-size: 14px; color: var(--text2); margin-bottom: 32px; line-height: 1.6; }
#welcome-input {
  width: 100%; padding: 12px 16px; border-radius: var(--radius); font-size: 15px;
  border: 1px solid var(--border2); background: var(--bg2); color: var(--text);
  outline: none; font-family: inherit; margin-bottom: 12px; text-align: center;
}
#welcome-input:focus { border-color: var(--text); }
#welcome-btn {
  width: 100%; padding: 12px; border-radius: var(--radius); font-size: 14px; font-weight: 600;
...
#library-header {
  padding: 20px 32px 0; background: var(--bg); border-bottom: 1px solid var(--border);
  flex-shrink: 0;
}
#library-header-top {
  display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px;
}
#library-greeting { font-size: 22px; font-weight: 700; letter-spacing: -0.5px; }
#library-greeting span { color: var(--text2); font-weight: 400; font-size: 14px; margin-left: 8px; }
#add-piece-btn {
  padding: 8px 18px; border-radius: var(--radius); font-size: 13px; font-weight: 600;
  border: none; background: var(--text); color: var(--bg); cursor: pointer; font-family: inherit;
  transition: opacity 0.12s;
}
#add-piece-btn:hover { opacity: 0.85; }
#library-tabs { display: flex; gap: 0; }
.lib-tab {
  padding: 10px 16px; font-size: 13px; cursor: pointer; color: var(--text2);
  border-bottom: 2px solid transparent; transition: all 0.12s; font-family: inherit;
  background: transparent; border-top: none; border-left: none; border-right: none;
}
.lib-tab.active { color: var(--text); font-weight: 600; border-bottom-color: var(--text); }
#library-body { flex: 1; overflow-y: auto; padding: 28px 32px; }
...
.piece-card:hover { border-color: var(--text); box-shadow: 0 2px 12px rgba(0,0,0,0.08); transform: translateY(-1px); }
.piece-card-composer { font-size: 11px; color: var(--text2); text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 6px; font-weight: 600; }
.piece-card-title { font-size: 14px; font-weight: 600; line-height: 1.4; margin-bottom: 4px; }
.piece-card-opus { font-size: 12px; color: var(--text2); }
.piece-card-badge {
  display: inline-block; margin-top: 10px; padding: 2px 8px; border-radius: 10px;
  font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.3px;
  background: var(--bg2); color: var(--text3);
}
.piece-card-badge.curated { background: #deeefa; color: #0a3d6b; }
.add-card {
  background: var(--bg2); border: 1.5px dashed var(--border2); border-radius: var(--radius-lg);
  padding: 20px; cursor: pointer; transition: all 0.15s; display: flex;
  flex-direction: column; align-items: center; justify-content: center; gap: 8px; min-height: 120px;
}
.add-card:hover { border-color: var(--text); background: var(--bg); }
.add-card-icon { font-size: 24px; color: var(--text3); }
.add-card-label { font-size: 13px; color: var(--text2); font-weight: 500; }
...
#chord-strip-header {
  ...
  padding: 7px 16px 4px; flex-wrap: wrap; border-bottom: 1px solid var(--border);
}
.strip-label { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; color: var(--text3); margin-right: 4px; }
.legend-item { display: flex; align-items: center; gap: 4px; font-size: 10px; color: var(--text2); }
.legend-dot { width: 7px; height: 7px; border-radius: 50%; }
#section-tabs { display: flex; overflow-x: auto; }
...
.section-tab:hover { color: var(--text); background: var(--bg2); }
.section-tab.active { color: var(--text); font-weight: 600; border-bottom-color: var(--text); }
#chord-row { display: flex; gap: 6px; padding: 8px 16px 10px; flex-wrap: wrap; }
.chord-chip {
...
.chord-chip.tonic     { background: #deeefa; color: #0a3d6b; border-color: #aad1f0; }
.chord-chip.dominant  { background: #fde8e8; color: #6b1414; border-color: #f5b8b8; }
.chord-chip.predominant { background: #fef3de; color: #5c3100; border-color: #f7c97a; }
.chord-chip.chromatic { background: #eeeafd; color: #30237a; border-color: #ccc5f5; }
...
<div id="welcome-screen">
  <div id="welcome-inner">
    <h1>Arioso</h1>
    <p>Your AI score companion.<br>What should we call you?</p>
...
<div id="library-screen">
  <div id="library-header">
    <div id="library-header-top">
      <div id="library-greeting">Welcome back <span id="greeting-name"></span></div>
...
<div id="top-bar">
  <button id="back-btn" onclick="goToLibrary()" style="padding:4px 10px;border-radius:var(--radius);font-size:11px;border:1px solid var(--border);background:var(--bg2);cursor:pointer;color:var(--text2);font-family:inherit;margin-right:4px;">&#8592; Library</button>
  ...
  <div id="score-panel">
    <div id="score-header">
      ...
      <div id="score-header-actions">
        ...
        <span id="zoom-label" style="font-size:11px;color:var(--text2);min-width:34px;text-align:center;">100%</span>
    ...
    <div id="chord-strip-container">
      ...
      <div style="padding:3px 16px 7px;font-size:10px;color:var(--text3);">AI-generated analysis  --  click any chord to see reasoning. Override if incorrect.</div>
    </div>
...
function placeMeasureLabel() {
  ...
  var canvas = document.getElementById("pc-" + pendingMeasurePage);
  ...
  label.style.top = (pendingMeasureY) + "px";
...
function renderLibrary() {
  ...
  addCard.innerHTML = "<div class='add-card-icon'>+</div><div class='add-card-label'>Add a piece</div>";
...
function openPiece(libIdx) {
  ...
  // Load PDF from IndexedDB using piece-specific key
  var pdfKey = libPiece.pdfKey || libPiece.id;
  loadPDFfromDBKey(pdfKey, function(buffer) {
    if (!buffer) {
      // No PDF saved yet  --  show upload zone
      document.getElementById("upload-zone").style.display = "block";
      document.getElementById("pdf-pages").style.display = "none";
      (function(){var b=document.getElementById("annotate-btn");if(b)b.style.display="none";})();
      document.getElementById("draw-btn").style.display = "none";
  ...
  { id:"chopin-ballade-1", composer:"Chopin", title:"Ballade No. 1", full:"Ballade No. 1 in G minor, Op. 23", year:"1835", key:"G minor",
    ...
    sections:[
      ...
      {label:"Second theme",measures:"mm. 37-67",page:"pp. 3-5",chords:[
        ...
        {symbol:"V7/IV",fn:"chromatic",note:"m.41  --  Secondary dominant of Ab. Chopin immediately enriches the new key with color. The paradise is not simple."},
        {symbol:"IV",fn:"predominant",note:"m.43  --  Ab major, subdominant of Eb. The harmony expands outward, warm and unhurried."},
        {symbol:"I",fn:"tonic",note:"m.45  --  Eb major, confirmed and singing. Let the melody float above a deep, resonant bass."},
        {symbol:"V7/V",fn:"chromatic",note:"m.50  --  Secondary dominant of the dominant. A brief intensification before settling back."},
        {symbol:"V7",fn:"dominant",note:"m.52  --  Dominant of Eb. The first hint of restlessness in this paradise. Resolution comes, but feels earned."},
        {symbol:"I",fn:"tonic",note:"m.54  --  Eb major returns. But listen: the inner voices have chromatic motion now. Something is darkening."},
        {symbol:"ii",fn:"predominant",note:"m.58  --  F minor. The shadows have arrived at the edge of the garden. The tone changes perceptibly."},
        {symbol:"V7",fn:"dominant",note:"m.60  --  Dominant of Eb, but the resolution no longer feels certain. The harmonic ground is shifting."},
        {symbol:"I",fn:"tonic",note:"m.62  --  Eb major, but fragile now. Chopin is already preparing the listener for the destruction to come."},
        {symbol:"V7/i",fn:"chromatic",note:"m.65  --  Dominant seventh pulling back toward G minor. The second theme is being reclaimed by darkness."},
...
var SYS = "You are Arioso, an AI score companion for serious pianists. Your users are advanced performers  --  conservatory-trained or equivalent. Your writing adapts strictly to context. For ANALYSIS requests: write like a rigorous musicologist. Identify formal structures precisely (modified sonata, rondo, etc.), Roman numeral analysis, key relationships, and harmonic language. For Chopin specifically, the central analytical lens is the equilibrium of chromaticism and diatonicism  --  how Chopin suspends tonality through tonal ambiguity, delayed resolution, floating tonality, double tonic complex, and the Neapolitan as structural pillar rather than ornament. Cite specific scholars where relevant: Berger on form, Rothstein on thematic ambiguity, Samson on tonal expansion, Rink on the dialectic of analysis and history. Connect every analytical observation to what the performer must understand and do. For INTERPRETATIONS requests: discuss specific named recordings  --  pianist, specific interpretive choices at key passages, connection to harmonic structure. Include a YouTube search link for each pianist formatted as: [Listen: Name Year](https://www.youtube.com/results?search_query=Name+Piece+Year). Be opinionated about which recordings are essential. For HISTORY and CONTEXT requests: write like serious program notes  --  LA Phil register. For Chopin, include: the ballade form's origins from medieval French Ars Nova through German Romantic poets to Mickiewicz; Chopin as a bridge figure whose chromaticism anticipates Wagner and ultimately Schoenberg; Schumann's reaction; the biographical context. For ALL contexts: direct, specific, opinionated. No hedging, no passive voice, no markdown asterisks. 3-5 sentences unless more depth is requested. You have web search available. Source hierarchy is strict: (1) Peer-reviewed journal articles (Nineteenth Century Music, Music Analysis, Journal of the American Musicological Society, Music Theory Spectrum, Integral)  --  highest authority; (2) University press books (Oxford, Cambridge, Harvard, Norton)  --  high authority; (3) Established music encyclopedias (Grove Music Online, MGG)  --  acceptable; (4) Major orchestra program notes (LA Phil, Carnegie Hall, BBC Proms, Vienna Philharmonic)  --  acceptable for historical context; (5) Everything else  --  do not cite as a source. Never cite blogs, personal websites, or amateur analyses, even if they appear in search results. Always search before generating analysis, especially for pieces beyond the core canon. CRITICAL: Never mention your sources, limitations, or methodology to the user. Never ask the user which option they prefer. Just deliver the analysis directly and confidently. The user is a pianist, not a researcher.";
...
function selectPiece(i) {
  ...
  if(pdfDoc) (function(){var b=document.getElementById("annotate-btn");if(b)b.style.display="block";})();
  document.getElementById("chat-messages").innerHTML = "";
...
function renderPage(num, container) {
  ...
  pdfDoc.getPage(num).then(function(page) {
    ...
    (function(n, c) {
      c.onclick = function(e){ handleClick(e, n, c); };
    })(num, canvas);
  });
}
...
function analyzeClick(page, xp, yp) {
  var prompt = "The performer clicked on page " + page + " of " + piece.full + " by " + piece.composer + " (key: " + piece.key + ") at roughly " + xp + "% across and " + yp + "% down the page. Based on your knowledge of this piece, what harmony is most likely here? Respond ONLY in this exact JSON format with no other text: {\"roman\": \"V7\", \"function\": \"dominant\", \"explanation\": \"What this harmony is doing here and why it matters  --  2 short sentences, plain language.\"}. Function must be one of: tonic, dominant, predominant, chromatic.";
...
function loadAnalysis() {
  ...
  var prompt = "";
  if(tab==="story") {
    var sources = (piece.sources && piece.sources.analysis) ? " IMPORTANT  --  source priority: First search for peer-reviewed academic sources, journal articles, and university press scholarship. Use these specific sources: " + piece.sources.analysis + ". Only use non-academic sources (program notes, established music encyclopedias) as secondary support. Never cite blogs, amateur analyses, or unattributed web content as sources. Where multiple scholarly sources agree on a harmonic interpretation, state it with confidence and cite them. Where sources differ or where your analysis departs from existing analyses, flag this explicitly (e.g. 'Berger labels this V7/III, though Rothstein argues...')." : " Search for peer-reviewed scholarship and academic analyses of this piece. Prioritize JSTOR, Google Scholar, university press publications, and established music journals. Avoid blogs and amateur analyses.";
    prompt = "Write a structured analytical overview of " + piece.full + " by " + piece.composer + " in the following format. Start with a section labeled Overview: (2-3 sentences on formal structure, key architecture, and the composer's harmonic language  --  the big picture a performer needs before sitting down). Then write one short paragraph per major section of the piece, each labeled with the section name and measure range, e.g. Introduction (mm. 1-7): covering the key harmonic events, what is structurally significant, and what it demands from the performer. Be precise and analytical  --  use Roman numeral terminology, name specific chords and functions, cite scholarly perspectives where relevant. Cross-reference your analysis against existing published analyses and flag where interpretations differ. No markdown asterisks. Connect every observation to performance." + sources;
  } else if(tab==="recording") {
    prompt = "Write a structured guide to the major recordings of " + piece.full + ". Start with a section labeled Overview: (1-2 sentences on what defines the interpretive tradition of this piece and what separates great recordings from the rest). Then write one paragraph per major recording, each labeled with the pianist name and year as a heading, e.g. Martha Argerich (1975):  --  covering their specific interpretive choices at key passages, how those choices connect to the harmonic structure, and what this recording uniquely reveals. Be opinionated. For each recording, include a YouTube search link formatted as: [Listen on YouTube](https://www.youtube.com/results?search_query=" + encodeURIComponent(piece.composer + " " + piece.title) + "+pianist+year). Cover at least 3 recordings. No markdown asterisks.";
  } else {
    prompt = "Write a structured practice guide for " + piece.full + " by " + piece.composer + ". Start with a section labeled Overview: (1-2 sentences on the core technical and musical challenges of this piece). Then write one paragraph per major practice priority, each labeled with a descriptive heading and measure range, e.g. The Neapolitan passages (mm. 28-30):  --  naming the specific challenge and giving actionable instruction grounded in the harmonic structure. Cover 3-4 priorities. No generic advice. No markdown asterisks.";
  }
  callAPIWithSearch([{role:"user",content:prompt}], function(text) {
    content.innerHTML = renderSectioned(text);
  });
}
...
function renderSectioned(text) {
  var html = text
    ...
    .replace(/\n/g, " ");
  html = html.replace(/([A-Z][A-Za-z0-9 &().,]+):/g, "<strong style='color:var(--text);display:block;margin-top:16px;margin-bottom:5px;font-size:13px;'>$1</strong>");
...
function loadStudy() {
  ...
  content.innerHTML = "<div class='loading-wrap'><div class='ldots'><span></span><span></span><span></span></div></div>";
  var historySources = (piece.sources && piece.sources.history) ? " Priority scholarly sources: " + piece.sources.history + ". Use academic biographies and peer-reviewed historical scholarship. Program notes from major orchestras (LA Phil, Carnegie Hall, BBC Proms) are acceptable secondary sources. Avoid Wikipedia as primary source." : " Search for academic biographies, peer-reviewed historical scholarship, and major orchestra program notes for this piece. Avoid Wikipedia as primary source.";
  var scholarSources = (piece.sources && piece.sources.scholarship) ? " Priority scholarly sources: " + piece.sources.scholarship + ". Search JSTOR and Google Scholar for additional peer-reviewed work. Cite journal names, years, and authors specifically." : " Search JSTOR, Google Scholar, and Music Index for peer-reviewed musicological scholarship on this piece. Cite journal names, years, and authors specifically.";
  var ps = {
    history: "Write a structured historical and contextual overview of " + piece.full + " by " + piece.composer + " in the style of serious concert program notes  --  like the LA Phil or Carnegie Hall would publish." + historySources + " Use the following sections with bold headings: Origins: (when composed, circumstances, what prompted it); Composer at the Time: (life circumstances  --  health, relationships, exile, philosophy); Cultural Moment: (broader historical and aesthetic context); Reception and Legacy: (how received, what contemporaries said, why it matters). Each section 2-3 sentences. Cite sources where you find them. Authoritative, accessible, specific. No markdown asterisks.",
    scholarship: "Write a structured overview of the musicological scholarship on " + piece.full + " by " + piece.composer + "." + scholarSources + " Use the following sections with bold headings: Formal Analysis: (key scholarly debates about structure and form, citing specific analysts and works); Harmonic Language: (what analysts say about harmonic innovation, citing specific publications); Programmatic Content: (debates about narrative or programmatic interpretation); Performance Practice: (what scholarship says about interpretation). Each section 2-3 sentences. Name specific analysts, journals, and published works. No markdown asterisks.",
    interviews: "Write a structured compilation of documented statements about " + piece.full + ". Search for interviews, essays, and documented master class remarks from major performers of this piece. Use the following sections with bold headings: The Composer's Voice: (letters, diary entries, documented statements  --  if none exist, say so); Contemporary Reactions: (what other composers or critics said); The Great Interpreters: (what major performers have said  --  name specific people and quote or paraphrase specifically, with source where known). Each section 2-3 sentences. No markdown asterisks."
  };
  callAPIWithSearch([{role:"user",content:ps[studyTab]}], function(text){ content.innerHTML = renderSectioned(text); });
}
...
/* API */
function callAPI(messages, callback, useSearch) {
  var body = {
  ...
  fetch("/api/chat", {
    ...
    // Extract text from all content blocks (handles tool_use + text responses)
    var text = "";
...
// Search-enabled version - falls back to regular if search fails
function callAPIWithSearch(messages, callback) {
  // Use regular call for reliability - prompts guide quality
  callAPI(messages, callback, false);
}
...
function renderChordStrip() {
  if (!piece || !piece.sections || !piece.sections.length) {
    document.getElementById("chord-strip-container").style.display = "none";
    return;
  }
...
function renderSectionTabs() {
  ...
  piece.sections.forEach(function(s, i) {
    ...
    tab.textContent = s.label + "   .   " + s.measures + (s.page ? "   .   " + s.page : "");
...
function loadChordAnalysis(chord) {
  ...
  var content = document.getElementById("analysis-content");
  content.innerHTML = "<div class='loading-wrap'><div class='ldots'><span></span><span></span><span></span></div></div>";
  var prompt = "Analyze the chord " + chord.symbol + " (" + chord.fn + " function) at " + chord.note.split(" -- ")[0].trim() + " in " + piece.full + " by " + piece.composer + ". Explain: what this harmony is doing structurally in this passage, how it relates to the surrounding harmonic context, and what it demands from the performer in terms of voicing, timing, or touch. Be precise and analytical, like a musicologist who also performs. 2-3 sentences.";
  callAPIWithSearch([{role:"user",content:prompt}], function(text) {
    content.innerHTML = "";
...
function applyZoom() {
  if (!pdfDoc) return;
  // Re-render each page with new zoom level
  for (var i = 1; i <= pdfDoc.numPages; i++) {
    (function(pageNum) {
      pdfDoc.getPage(pageNum).then(function(page) {
        ...
        var scale = (baseWidth / page.getViewport({scale: 1}).width) * zoomLevel;
        var vp = page.getViewport({scale: scale});
...
function toggleDraw() {
  ...
  document.querySelectorAll(".pdf-page-wrapper canvas:not(.draw-canvas)").forEach(function(c) {
    c.style.cursor = drawMode ? "none" : "default";
  });
}
...
function startDraw(e, dc) {
  ...
  currentDrawCtx.moveTo((e.clientX - rect.left) * scaleX, (e.clientY - rect.top) * scaleY);
  currentDrawCtx.strokeStyle = "#e05050";
  currentDrawCtx.lineWidth = 2;
...
function continueDraw(e, dc) {
  ...
  var scaleX = dc.width / rect.width;
  var scaleY = dc.height / rect.height;
  currentDrawCtx.lineTo((e.clientX - rect.left) * scaleX, (e.clientY - rect.top) * scaleY);
